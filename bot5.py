"""Halite tournament bot, fifth generation.

bot2's engine plus one idea grown from Gavin's heat-seeking gradient bot:
GRADED REPULSION. bot2's danger model is a hard wall (a cell adjacent to a
lighter enemy is forbidden, everything else is fine), so ships mine calmly
at distance 2 from a pirate and only react at the cliff edge. bot5 adds a
soft, cargo-relative threat gradient to movement scoring: every enemy ship
light enough to beat us in a collision pushes on nearby cells with strength
falling off over 3 steps. Returning miners route around pirate country
before there is an emergency, and the pressure is phase-scaled -- stronger
in the early game, where a single robbery snowballs (measured: mirror games
are bimodal), relaxing once the economy is established.

The hard wall stays underneath; the gradient only shapes choices among
already-survivable moves.

Inherited from bot2 (see docs/bot2.md):
...
def agent(obs, config):
    ...
    # ---- Movement resolution ------------------------------------------------
    repulsion_weight = (
        REPULSION_EARLY if board.step < EARLY_PHASE_STEP else REPULSION_LATE
    )
    reserved = set()
