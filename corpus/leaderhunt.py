...
HUNT_WEIGHT = 0.7              # hunting score multiplier vs mining
HUNTERS_PER_PREY = 2           # pack size allowed on one target
LEAD_MIN_STEP = 60             # don't judge a 'runaway' before this
LEAD_RATIO = 1.15              # opponent wealth/ours that counts as runaway
LEADER_PREY_MIN = 40           # chase leader ships down to this cargo
LEADER_HUNT_BOOST = 3.0        # extra hunt weight on the runaway leader
LEADER_HUNTERS_PER_PREY = 3    # bigger packs on the leader's ships
...
def agent(obs, config):
    ...
    # Runaway-leader detection: if one opponent's total wealth (bank + cargo)
    # clearly leads ours past the opening, gang up on THAT player -- a
    # peaceful economy wins by never losing its laden ships, so forcing it
    # to lose them removes its whole edge.
    leader_positions = set()
    if board.step >= LEAD_MIN_STEP and board.opponents:
        my_total = me.halite + sum(s.halite for s in me.ships)
        leader = max(board.opponents,
                     key=lambda p: p.halite + sum(s.halite for s in p.ships))
        leader_total = leader.halite + sum(s.halite for s in leader.ships)
        if leader_total > my_total * LEAD_RATIO:
            leader_positions = {s.position for s in leader.ships}
    ...
    prey = [(ep, ec) for ep, ec in enemy_ships
            if ec >= HUNT_MIN_PREY_CARGO
            or (ep in leader_positions and ec >= LEADER_PREY_MIN)]
    ...
    for ship in free_ships:
        ...
        if cargo == 0:
            for prey_pos, prey_cargo in prey:
                ...
                boost = LEADER_HUNT_BOOST if prey_pos in leader_positions else 1.0
                score = HUNT_WEIGHT * boost * prey_cargo / (1 + d)
    ...
    for score, ship_id, (kind, tpos) in bids:
        ...
        if kind == "mine":
            ...
            cap = LEADER_HUNTERS_PER_PREY if tpos in leader_positions else HUNTERS_PER_PREY
            if hunter_counts.get(tpos, 0) >= cap:
                continue
