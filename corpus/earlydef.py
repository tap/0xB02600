...
EARLY_RETURN_THRESHOLD = 300   # bank sooner while a loss would snowball
EARLY_DANGER_RADIUS = 2        # laden ships keep this far from lighter enemies early
...
def agent(obs, config):
    ...
    def dangerous(pos, cargo):
        radius = (
            EARLY_DANGER_RADIUS
            if board.step < EARLY_PHASE_STEP and cargo >= 200
            else 1
        )
        return any(
            ec <= cargo and toroidal_distance(ep, pos, size) <= radius
    ...
    if board.step < EARLY_PHASE_STEP:
        return_threshold = EARLY_RETURN_THRESHOLD
    elif steps_left > 100:
        return_threshold = CARGO_RETURN_THRESHOLD
    else:
        return_threshold = LATE_RETURN_THRESHOLD
