...
def agent(obs, config):
    ...
    # Never idle on top of our own yard: step toward the richest neighbor.
    for ship in me.ships:
        if ship.next_action is None and any(
            ship.position == sy.position for sy in me.shipyards
        ):
            options = {
                ShipAction.NORTH: ship.cell.north.halite,
                ShipAction.EAST: ship.cell.east.halite,
                ShipAction.SOUTH: ship.cell.south.halite,
                ShipAction.WEST: ship.cell.west.halite,
            }
            ship.next_action = max(options, key=options.get)
    ...
    if board.step < SPAWN_UNTIL_STEP:
        for shipyard in me.shipyards:
            ...
            if any(s.position == shipyard.position for s in me.ships):
                continue  # a ship is parked on the yard; spawning would kill both
